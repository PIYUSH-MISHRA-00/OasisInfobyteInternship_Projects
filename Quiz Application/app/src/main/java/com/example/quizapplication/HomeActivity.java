...
import com.example.quizapplication.api.Api;
import com.example.quizapplication.api.ApiCount;
import com.example.quizapplication.api.QuizQuestions;
import com.example.quizapplication.api.Result;
import com.example.quizapplication.question.Question;
		...
		call.enqueue(new Callback<QuizQuestions>() {
			...
			public void onResponse(Call<QuizQuestions> call, Response<QuizQuestions> response) {
				if (response.isSuccessful()) {
					...
					// Handle the case when the response is not successful (e.g., show an error message)
					Toast.makeText(getApplicationContext(), "Failed to fetch quiz questions", Toast.LENGTH_SHORT).show();
					progressBar.setVisibility(View.INVISIBLE);
					start.setClickable(true);
				}
			}
